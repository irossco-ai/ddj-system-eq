...
- **4-band modes** with the channel-EQ figures of the Pioneer **DJM-V10** and
  Allen & Heath **Xone:96** / **Xone:92 Mk2**: the CFX knob becomes **LO**,
  LOW → **LO MID**, MID → **HI MID**, HI → **HI** (the DJ filter is off while
  a 4-band mode is selected).
...
| `eq_modes.xone96.bands` | LO 180 Hz +6/−∞ · LO MID 350 Hz +10/−27 · HI MID 1.1 kHz +10/−27 · HI 3 kHz +6/−∞ | Xone:96 channel EQ, **matched to the response graphs in the A&H user guide** (within ~1 dB): the kills are gentle 12 dB/oct filters (`kill_edge_fc` 320 Hz / 2 kHz, `kill_edge_q`, `shelf_cap_db`), the +6 shelves reach full gain by ~100 Hz / ~3 kHz (`boost_fc`), and the mid bells are asymmetric — narrow boost `q_boost` 1.3, wide cut `q_cut` 0.8. `four_band: true` puts LO on the CFX knob |
| `eq_modes.xone92.bands` | LO 220 Hz +6/−∞ · LM 320 Hz +6/−30 · HM 1.8 kHz +6/−30 · HI 2.4 kHz +6/−∞ | Xone:92 Mk2 (A&H spec sheet) with the 96's shapes scaled to its frequencies — extrapolated, no 92 graphs to hand |
...
| *(−∞ kills)* | — | Any −∞-type LOW/HI band is a shelf down to `shelf_deep_db` (−24 default), then a high/low-pass sweeps in to `kill_edge_fc` as the knob reaches kill while the shelf relaxes to `shelf_cap_db`. Pioneer isolator modes use a steep 24 dB/oct edge at the crossover; the Xone modes use a gentle single 12 dB/oct edge above the crossover, as the A&H graphs show. The isolator MID is four staggered bells (`centres`, `bell_scale`) forming a flat-topped 200–5000 Hz band. All tuned from computed frequency responses and checked for monotonic knob travel — not by ear |
